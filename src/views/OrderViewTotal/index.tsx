...
const OrderViewTotal = () => {
    ...
    return (
        <div>
                    ...
                    cartValue.map((element, index) => {
                        return (
                            <li key={index + 1} className={styles.orderViewTotalListItem}>
                                <span>{element.category} {element.name} x {element.quantity}</span>
            ...
            <hr />
            <p className={styles.orderViewTotalContent}><span className={styles.orderViewTotalContentHeader}>Total</span><span className={styles.orderViewTotalText}>{cartTotal}</span></p>
            <div className={styles.orderViewTotalButton}>
                <Button variant='primary' type="button" label="place order" />
            </div>
        </div>
    )
}
